...
class TransT(nn.Module):
    ...
    def forward(self, search, template):
        ...
        hs_cls = hs.reshape(32, 32, -1, 256).permute(2, 3, 0, 1)
        hs_cls = self.conv1(hs_cls)
        hs_cls = hs_cls.permute(2, 3, 0, 1).reshape(1024, -1, 256).unsqueeze(0).transpose(1, 2)

        # hs_cls = hs.unsqueeze(0).transpose(1, 2)
        outputs_class = self.class_embed(hs_cls)
        # bs, c, h, w = x.shape
        hs = hs.reshape(32, 32, -1, 256).permute(2, 3, 0, 1)
        hs = self.cot(hs)
        hs = hs.permute(2, 3, 0, 1).reshape(1024, -1, 256).unsqueeze(0).transpose(1, 2)
        outputs_coord = self.bbox_embed(hs).sigmoid()  # (1, 19, 1024, 4)
